...
import { mens_shoes } from "../component/body.js"
...
container.innerHTML=mens_shoes()
...
for(let el of sort){
    el.addEventListener("click",function(){
        let color=Math.floor(Math.random() * 10) +1
        cSearch("mens_shoes",el.id,color)
    })
}
...
for(let el of color){
    el.addEventListener("click",function(){
        cSearch("mens_shoes","oldest",el.id)
    })
}
